$(function () {
    ...
    function validateFormElements(input) {
        ...
        if($(input).attr("type") === "text" && $(input).prop("required")) {
            ...
            if($(input).data("id") === "cc-name") {
                min = 5;
                validateInputValue(input,`Måste bestå av ett giltigt förnamn och efternamn`, min);

            } else if($(input).data("id") === "cc-number") {
                min = 16;
                validateInputValue(input,`Måste bestå av ${min} antal siffror utan bindestreck (-)`, min);

            } else if($(input).data("id") === "cc-expiraton-month") {
                min = 2;
                validateInputValue(input,`Måste bestå av ${min} antal siffror`, min);

            } else if($(input).data("id") === "cc-expiraton-year") {
                min = 2;
                validateInputValue(input,`Måste bestå av ${min} antal siffror`, min);

            } else if($(input).data("id") === "cc-cvv") {
                min = 3;
                validateInputValue(input,`Måste bestå av ${min} antal siffror`, min);
